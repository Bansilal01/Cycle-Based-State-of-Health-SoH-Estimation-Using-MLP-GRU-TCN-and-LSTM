...
# Load data
mat = scipy.io.loadmat('B0007.mat')
capacities = extract_capacity(mat['B0007'])
...
plt.grid(True)
plt.tight_layout()
plt.savefig(f"{output_dir}/prediction_comparison_all_models.png", dpi=600)
plt.close()
...
for name, res in results.items():
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{name}_loss_curve.png", dpi=600)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{name}_predicted_vs_actual.png", dpi=600)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{name}_residuals_histogram.png", dpi=600)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{name}_actual_vs_pred_scatter.png", dpi=600)
    plt.close()
...
summary_df.round(4).to_csv(f"{output_dir}/model_evaluation_summary.csv")
